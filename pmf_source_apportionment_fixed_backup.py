...
class MMFPMFAnalyzer:
    def __init__(self, station, start_date=None, end_date=None, output_dir="pmf_results"):
        """
    ...
    def create_pmf_dashboard(self):
        ...
        # Create a context manager to capture and suppress stdout during plot generation
        # This prevents print statements in plot functions from being displayed in HTML
        import io
        import sys
        ...
        # Use the output capture to suppress print statements that might leak into HTML
        with CaptureOutput() as capture:
            ...
            plt.savefig(plot_path, dpi=300, bbox_inches='tight', facecolor='white')
                ...
                # print statements will be captured during plot creation
                
            except Exception as e:
                # Errors will still show up since we want to see them
                pass
    ...
    def _create_html_dashboard(self, plot_files):
        ...
        # Output redirect will be undone after with block exits
        # This print will show after output capture is released
...
""")
    ...
    def _create_diagnostic_scatters(self, dashboard_dir, plot_files, F_profiles, G_contributions):
        ...
        # Mark potential outliers - ensure no array values leak to output
        try:
            ...
            if np.any(outlier_mask):
                # Ensure array slicing doesn't display values
                outlier_leverage = leverage[outlier_mask].copy()
                outlier_residuals = residual_norm[outlier_mask].copy()
                ax4.scatter(outlier_leverage, outlier_residuals, 
                           c='red', s=60, marker='x', label='Potential Outliers')
                ax4.legend()
        except Exception as e:
            # If outlier detection fails, skip it to avoid any output issues
            pass
    ...
    def _create_wind_analysis_plots(self, dashboard_dir, plot_files, G_contributions):
        ...
        fig.suptitle(f'{self.station} PMF Factors vs Wind Conditions', fontsize=18, fontweight='bold')
        ...
        # Calculate wind-factor correlations to select top factors for polar plots
        top_factors = []
        if wind_dir_col and not wind_df['wind_dir'].isna().all():
            ...
            for f in range(self.factors):
                ...
                if np.sum(valid_mask) > 10:  # Need at least 10 points for meaningful correlation
                    corr = np.abs(np.corrcoef(wind_df.loc[valid_mask, 'wind_dir'], 
                                             G_wind[valid_mask, f])[0, 1])
            ...
            if wind_speed_col and not wind_df['wind_speed'].isna().all():
                ...
                for f in range(self.factors):
                    ...
                    if np.sum(valid_mask) > 10:
                        corr = np.abs(np.corrcoef(wind_df.loc[valid_mask, 'wind_speed'], 
                                                 G_wind[valid_mask, f])[0, 1])
                        if not np.isnan(corr):
                            # Add to existing correlation or create new entry
                            existing_idx = next((i for i, (idx, _) in enumerate(factor_wind_correlations) if idx == f), None)
                            if existing_idx is not None:
                                # Take maximum of wind direction and wind speed correlation
                                factor_wind_correlations[existing_idx] = (f, max(factor_wind_correlations[existing_idx][1], corr))
                            else:
                                factor_wind_correlations.append((f, corr))
            
            # Sort by correlation strength and select top 3
            factor_wind_correlations.sort(key=lambda x: x[1], reverse=True)
            top_factors = [idx for idx, corr in factor_wind_correlations[:min(3, len(factor_wind_correlations))]]
            
            print(f"   🎯 Top wind-correlated factors: {[(f+1, corr) for f, corr in factor_wind_correlations[:3]]}")
        
        # Plot 3, 4, & 5: Factor contributions vs wind direction (polar plots) - Top 3 factors
        if wind_dir_col and not wind_df['wind_dir'].isna().all() and top_factors:
            # Show top 3 wind-correlated factors in polar plots
            n_polar_plots = min(3, len(top_factors))
            
            # Adjust grid layout to accommodate up to 3 polar plots
            if n_polar_plots == 1:
                polar_positions = [(0, 2)]
            elif n_polar_plots == 2:
                polar_positions = [(0, 2), (0, 3)]
            else:  # 3 plots
                # Adjust the main grid to have more columns if needed
                gs = fig.add_gridspec(3, 5, height_ratios=[1, 1, 1], width_ratios=[1, 1, 0.8, 0.8, 0.8])
                polar_positions = [(0, 2), (0, 3), (0, 4)]
                
                # Need to recreate earlier plots with new grid
                ax1.remove()
                ax2.remove()
                
                # Recreate Plot 1: Wind Rose
                ax1 = fig.add_subplot(gs[0, 0])
                if wind_dir_col and not wind_df['wind_dir'].isna().all():
                    wind_dirs = wind_df['wind_dir'].dropna()
                    if len(wind_dirs) > 0:
                ...
                # Recreate Plot 2: Wind Speed Distribution
                ax2 = fig.add_subplot(gs[0, 1])
                if wind_speed_col and not wind_df['wind_speed'].isna().all():
                    wind_speeds = wind_df['wind_speed'].dropna()
                    if len(wind_speeds) > 0:
            ...
            # Create polar plots for top factors
            for plot_idx, factor_idx in enumerate(top_factors[:n_polar_plots]):
                pos_row, pos_col = polar_positions[plot_idx]
                ax_polar = fig.add_subplot(gs[pos_row, pos_col], projection='polar')
                
                # Get wind direction and factor contributions
                valid_mask = ~(wind_df['wind_dir'].isna() | np.isnan(G_wind[:, factor_idx]))
                if np.sum(valid_mask) > 5:
                    wd = wind_df.loc[valid_mask, 'wind_dir'].values
                    fc = G_wind[valid_mask, factor_idx]
                    
                    # Convert degrees to radians
                    wd_rad = np.radians(wd)
                    
                    # Create polar scatter plot with factor-specific color
                    scatter = ax_polar.scatter(wd_rad, fc, c=fc, cmap='viridis', 
                                             alpha=0.6, s=30)
                    
                    # Get correlation for title
                    factor_corr = next((corr for idx, corr in factor_wind_correlations if idx == factor_idx), 0)
                    ax_polar.set_title(f'Factor {factor_idx + 1} vs Wind Direction\n(|r|={factor_corr:.2f})', 
                                      fontweight='bold', pad=20, fontsize=10)
                    ax_polar.set_theta_zero_location('N')
                    ax_polar.set_theta_direction(-1)
                    
                    # Add colorbar
                    plt.colorbar(scatter, ax=ax_polar, shrink=0.6, pad=0.1)
        
        # Plot 5: Factor contributions binned by wind direction (middle row)
        ...
        # Plot 6: Factor contributions vs wind speed (middle right)
        ...
        if wind_speed_col and not wind_df['wind_speed'].isna().all():
            ...
            for f in range(self.factors):
                ...
                if len(ws) > 10:
                    ...
                    ax6.plot(ws_trend, p(ws_trend), '--', color=colors[f], alpha=0.7)
        ...
        # Plot 7: Wind speed binned analysis (bottom left)
        ...
        # Plot 8: Correlation matrix (bottom right)
        ...
        if len(corr_data.columns) > self.factors:
            ...
            if wind_cols and factor_cols:
                subset_corr = corr_matrix.loc[wind_cols, factor_cols]
    ...
    def convert_dashboard_to_pdf(self, dashboard_dir, station=None):
        ...
        html_files = list(dashboard_dir.glob("*_pmf_dashboard.html"))
        
        if not html_files:
            print(f"   ⚠️ No HTML dashboard found in {dashboard_dir}")
            return None
    ...
    try:
        ...
        if pmf.run_pmf_analysis():
            ...
            if args.create_pdf:
                ...
                # Look for HTML dashboard in the main output directory first
                html_files = list(pmf.output_dir.glob("*_pmf_dashboard.html"))
                if html_files:
                    dashboard_dir = pmf.output_dir
                else:
                    # Fallback to dashboard subdirectory
                    dashboard_dir = pmf.output_dir / "dashboard"
